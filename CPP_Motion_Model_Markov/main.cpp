...
float motion_model(float pseudo_position, float movement, vector<float> priors,
  ...
  for (float i = 0; i < map_size; ++i) 
  {
      float pre_pseudo_position = i;
      float delta_position=pseudo_position-pre_pseudo_position;
      float transition_probability = Helpers::normpdf(delta_position, control_stdev, movement);
      position_prob=position_prob+transition_probability*priors[i];
  }
